...
def fetch_article_data(article_url):
    try:
        ...
        response = requests.get(article_url, headers=headers, cert=pem_cert_path)
        response.raise_for_status()
...
def scrape_worldnews24_articles(url):
    try:
        ...
        response = requests.get(url, headers=headers, cert=pem_cert_path)
        response.raise_for_status()
